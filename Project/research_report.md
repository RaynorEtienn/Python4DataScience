...
## 1. Baseline Analysis (Definition of "All-Time High" / ATH)
*   **ATH definition:** The term "All-Time High (ATH)" in this report refers to the highest public Kaggle F1-Score observed at a given point in the experiment timeline. We record both the original baseline ATH and any later runs that exceed it, with experiment IDs for traceability.
*   **Original baseline ATH (pre-experiments):** **0.64812** — this is the score we used as the initial benchmark.
*   **Note on later results:** Subsequent experiments changed the leaderboard. For example, **Experiment 22** (Super Ensemble with Bagging) achieved **0.65381**, which became the new ATH at that time. All reported ATH references in this document indicate the highest value observed up to that experiment.
*   **Strategy (original baseline):** Stacking Ensemble (XGBoost, LightGBM, CatBoost) + Logistic Regression Meta-Learner.
...
| **24** | **Selective Voting (Full Tune)** | Tuned ensemble including `BaggingClassifier(LogisticRegression)`, `RandomForest`, and boosters (Optuna; trials=30). Controlled with `ACTIVE_MODELS`. | 0.33004 | **0.64990** | **Promising**. Beat baseline ATH (0.64812) but below Exp 22 (0.65381). |
